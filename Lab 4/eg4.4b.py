...
# fetch the data and prepare the training and test sets
...
# define the DNN model and the training and test loops
class Net(nn.Module):
...
def my_train(hidden_layers):
    ...
    model = Net(no_features, no_labels, no_hidden, hidden_layers)
...
# run the training and plot the learning curves and the test error vs. the depth of DNN
...
def main():
  ...
  plt.legend()
  plt.savefig(os.path.join(figures_dir, '4.4b_1.png'))
  plt.close()
  ...
  plt.savefig(os.path.join(figures_dir, '4.4b_2.png'))
  plt.close()
